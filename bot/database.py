...
# Synchronous Supabase client (supabase-py v2 is sync; we run in executor)
...
async def get_user(telegram_id: int) -> Optional[Dict]:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("users").select("*").eq("telegram_id", telegram_id).execute()
    )
...
async def upsert_user(telegram_id: int, username: str, first_name: str, last_name: str,
    ...
    if existing:
        ...
        res = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: _client.table("users").insert(data).execute()
        )
        return res.data[0]
...
async def mark_channel_joined(telegram_id: int) -> None:
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("users").update({"channel_joined": True}).eq("telegram_id", telegram_id).execute()
    )
...
async def ban_user(telegram_id: int) -> None:
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("users").update({"is_banned": True}).eq("telegram_id", telegram_id).execute()
    )
...
async def unban_user(telegram_id: int) -> None:
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("users").update({"is_banned": False}).eq("telegram_id", telegram_id).execute()
    )


async def list_users(limit: int = 100, offset: int = 0,
                      search: Optional[str] = None, banned_only: bool = False) -> List[Dict]:
    q = _client.table("users").select("*", count="exact")
    if banned_only:
        q = q.eq("is_banned", True)
    if search:
        q = q.or_(f"username.ilike.%{search}%,first_name.ilike.%{search}%,telegram_id.eq.{search if search.isdigit() else 0}")
...
async def count_users() -> int:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("users").select("id", count="exact").execute()
    )
...
async def get_all_user_ids() -> List[int]:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("users").select("telegram_id").eq("is_banned", False).execute()
    )
...
async def create_order(user_id: int, service: str, details: Dict[str, Any],
    ...
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("orders").insert(data).execute()
    )
    return res.data[0]
...
async def get_order(order_id: str) -> Optional[Dict]:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("orders").select("*").eq("order_id", order_id).execute()
    )
    return res.data[0] if res.data else None
...
async def update_order_status(order_id: str, status: str) -> Dict:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("orders").update({
...
async def list_orders(limit: int = 50, offset: int = 0,
    ...
    q = _client.table("orders").select("*", count="exact")
    if status:
...
async def count_orders(status: Optional[str] = None) -> int:
    q = _client.table("orders").select("id", count="exact")
    if status:
        q = q.eq("status", status)
    res = await asyncio.get_event_loop().run_in_executor(None, lambda: q.execute())
    return res.count or 0
...
async def get_revenue_stats() -> Dict:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("orders").select("amount_bdt,service,status,created_at")
...
async def _refresh_settings() -> None:
    ...
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("settings").select("*").execute()
    )
...
async def set_setting(key: str, value: str) -> None:
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("settings").upsert({"key": key, "value": value}).execute()
    )
...
async def create_referral_link(code: str, label: str, created_info: str = "") -> Dict:
    ...
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("referral_links").insert(data).execute()
    )
    return res.data[0]
...
async def get_referral_link(code: str) -> Optional[Dict]:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("referral_links").select("*").eq("code", code).execute()
    )
    return res.data[0] if res.data else None
...
async def increment_referral_click(code: str, user_id: int) -> None:
    ...
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("referral_events").insert({
            "link_code": code,
            "user_id": user_id,
            "event_type": "click",
            "created_at": _now(),
    ...
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("referral_links").update(
...
async def increment_referral_conversion(code: str, user_id: int) -> None:
    ...
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("referral_links").update(
            {"conversions": (link["conversions"] or 0) + 1}
        ).eq("code", code).execute()
    )
...
async def list_referral_links() -> List[Dict]:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("referral_links").select("*").order("created_at", desc=True).execute()
    )
    return res.data
...
async def delete_referral_link(link_id: int) -> None:
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("referral_links").delete().eq("id", link_id).execute()
    )
...
async def create_broadcast(message_type: str, content: Dict, target: str) -> Dict:
    ...
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("broadcasts").insert(data).execute()
    )
    return res.data[0]
...
async def update_broadcast_stats(broadcast_id: int, success: int, fail: int) -> None:
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("broadcasts").update({
...
async def list_broadcasts(limit: int = 50) -> List[Dict]:
    res = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("broadcasts").select("*").order("sent_at", desc=True).limit(limit).execute()
    )
    return res.data
...
async def delete_broadcast(broadcast_id: int) -> None:
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: _client.table("broadcasts").delete().eq("id", broadcast_id).execute()
    )
